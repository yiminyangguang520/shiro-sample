...
import com.lee.shiro.shiro.MyUsernamePasswordToken;
import com.lee.shiro.util.MD5Utils;
import com.lee.shiro.constant.Constant;
...
public class MyAccessControlFilter extends AccessControlFilter {
  ...
  /**
   * 表示是否允许访问；mappedValue就是[urls]配置中拦截器参数部分，如果允许访问返回true，否则false； (感觉这里应该是对白名单（不需要登录的接口）放行的)
   * 如果isAccessAllowed返回true则onAccessDenied方法不会继续执行 这里可以用来判断一些不被通过的链接（个人备注） *
