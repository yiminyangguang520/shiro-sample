...
public class ShirConfig {
  ...
  /**
   * SecurityManager,安全管理器,所有与安全相关的操作都会与之进行交互; 它管理着所有Subject,所有Subject都绑定到SecurityManager,与Subject的所有交互都会委托给SecurityManager DefaultWebSecurityManager :
   * 会创建默认的DefaultSubjectDAO(它又会默认创建DefaultSessionStorageEvaluator) 会默认创建DefaultWebSubjectFactory 会默认创建ModularRealmAuthenticator
   */
  @Bean
